...
function formiraj_query($zahtjev)
{
    ...
    if (
        !empty($email_ured) || !empty($tel_tip_ured) || !empty($poz_br_ured)
        || !empty($broj_tel)
    ) {
        $ured = array();
...
{
    $url = "https://nominatim.openstreetmap.org/search.php?q=".urlencode($adresa)."&format=xml&email=antonio.kamber@fer.hr";
    $xml = simplexml_load_file($url) or die("Error: Nominatim ne može dohvatiti podatke");
